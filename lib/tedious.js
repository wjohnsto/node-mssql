    ...
    for (let index = 0, length = metadata.length; index < length; index++) {
        ...
        if (column.udtInfo) {
            ...
            if (DECLARATIONS[column.udtInfo.typeName]) {
                out[column.colName].type =
                    DECLARATIONS[column.udtInfo.typeName];
            }
        }
    }
            ...
            let timeout = setTimeout(() => {
                safeReject(
                    new base.ConnectionError(
                        'Tedious connection timed out after 10 seconds'
                    )
                );
            }, 10 * 1000);
  ...
  */
    ...
    _bulk(table, callback) {
        super._bulk(table, (err) => {
            ...
            if (!table.name) {
                return callback(
                    new base.RequestError(
                        'Table name must be specified for bulk insert.',
                        'ENAME'
                    )
                );
            }
            ...
            if (table.name.charAt(0) === '@') {
                return callback(
                    new base.RequestError(
                        "You can't use table variables for bulk insert.",
                        'ENAME'
                    )
                );
            }
            ...
            this.parent.acquire(this, (err, connection) => {
                ...
                debug(
                    'connection(%d): borrowed to request #%d',
                    IDS.get(connection),
                    IDS.get(this)
                );
                ...
                if (this.canceled) {
                    ...
                    this.parent.release(connection);
                    return callback(
                        new base.RequestError('Canceled.', 'ECANCEL')
                    );
                }
                ...
                connection.on(
                    'errorMessage',
                    (errorHandlers.errorMessage = handleError.bind(
                        null,
                        false,
                        connection
                    ))
                ...
                connection.on(
                    'error',
                    (errorHandlers.error = handleError.bind(
                        null,
                        true,
                        connection
                    ))
                ...
                if (table.create) {
                    const objectid = table.temporary
                        ? `tempdb..[${table.name}]`
                        : table.path;
                    const req = new tds.Request(
                        `if object_id('${objectid.replace(
                            /'/g,
                            "''"
                        )}') is null ${table.declare()}`,
                        (err) => {
        ...
        super._query(command, (err) => {
            ...
            this.parent.acquire(this, (err, connection, config) => {
                    ...
                    if (batchHasOutput) {
                        ...
                        for (let name in batchLastRow) {
                            ...
                            if (name !== '___return___') {
                                output[name] =
                                    value === tds.TYPES.Null ? null : value;
                            }
                        }
                    }
                ...
                req.on('columnMetadata', (metadata) => {
                    ...
                    if (
                        metadata.length === 1 &&
                        (metadata[0].colName === JSON_COLUMN_ID ||
                            metadata[0].colName === XML_COLUMN_ID)
                    ) {
                    ...
                    if (isChunkedRecordset) {
                        ...
                        if (
                            ...
                            try {
                                ...
                                row = null;
                                const ex2 = new base.RequestError(
                                    new Error(
                ...
                if (this._isBatch) {
                    if (Object.keys(this.parameters).length) {
                        for (let name in this.parameters) {
                            ...
                            let value = getTediousType(param.type).validate(
                                param.value
                            );

                            if (value instanceof TypeError) {
                                value = new base.RequestError(
                        ...
                        req.sqlTextOrProcedure = `declare ${declarations.join(
                            ', '
                        )};select ${assigns.join(', ')};${
                            req.sqlTextOrProcedure
                        };${
                            batchHasOutput
                                ? `select 1 as [___return___], ${selects.join(
                                      ', '
                                  )}`
                                : ''
                        }`;
                    ...
                    for (let name in this.parameters) {
                        ...
                        if (param.io === 1) {
                            req.addParameter(
                                param.name,
                                getTediousType(param.type),
                                parameterCorrection(param.value),
                                {
                                    length: param.length,
                                    scale: param.scale,
